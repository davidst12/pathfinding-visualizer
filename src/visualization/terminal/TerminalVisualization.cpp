...
TerminalVisualization::TerminalVisualization() : IVisualization() {}
...
TestModeSelectionResult TerminalVisualization::displayTestModeSelectionScreen() {
    ...
    // For simplicity in terminal mode, we will just set the test mode to SingleAlgorithm and continue.
    screen_result.test_mode = AlgorithmTestMode::SingleAlgorithm;
    screen_result.state_event = AppStateEvent::Continue;
    return screen_result;
}

AlgorithmSelectionResult TerminalVisualization::displayAlgorithmSelectionScreen(bool allow_multiple_selection) {
    ...
    do {
        ...
        switch(selection) {
            case '1':
                screen_result.algorithms_selected.insert(AlgorithmType::BFS);
                screen_result.state_event = AppStateEvent::Continue;
                break;
            case '2':
                screen_result.algorithms_selected.insert(AlgorithmType::DFS);
                screen_result.state_event = AppStateEvent::Continue;
                break;
            case '3':
                screen_result.algorithms_selected.insert(AlgorithmType::Dijkstra);
                screen_result.state_event = AppStateEvent::Continue;
                break;
            case '4':
                screen_result.algorithms_selected.insert(AlgorithmType::AStar);
                screen_result.state_event = AppStateEvent::Continue;
                break;
            case '5':
...
MapSelectionResult TerminalVisualization::displayMapSelectionScreen(std::map<std::string, Grid> maps) {
...
BasicScreenResult TerminalVisualization::displaySimulationScreen(std::vector<AlgorithmResult> results, bool wait_for_input) {
    BasicScreenResult screen_result;
