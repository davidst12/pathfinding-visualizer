...
TEST(AlgorithmResultTest, ToStringTest) {
    EXPECT_EQ("IDLE", state_to_string(AlgorithmState::kIdle));
    EXPECT_EQ("READY", state_to_string(AlgorithmState::kReady));
    EXPECT_EQ("RUNNING", state_to_string(AlgorithmState::kRunning));
    EXPECT_EQ("PATH_FOUND", state_to_string(AlgorithmState::kPathFound));
    EXPECT_EQ("PATH_NOT_FOUND", state_to_string(AlgorithmState::kPathNotFound));

    EXPECT_EQ("Dijkstra", algorithm_type_to_string(AlgorithmType::kDijkstra));
    EXPECT_EQ("BFS", algorithm_type_to_string(AlgorithmType::kBFS));
    EXPECT_EQ("DFS", algorithm_type_to_string(AlgorithmType::kDFS));
    EXPECT_EQ("A*", algorithm_type_to_string(AlgorithmType::kAStar));
}